        ...
        while (target != that) {
            if (target.tagName == "LI") {
                ...
                if (target.dataset.img) {
                    that.imgBlock.style.backgroundImage = "url(" + target.dataset.img + ")"
                };
                that.play();
...
AudioObjConsctructor.prototype.btnToggler = function () {
    this.btnPlay.classList.toggle("play");
    this.btnPlay.classList.toggle("stop");
    if (this.btnPlay.classList[1] == "stop") this.play();
    if (this.btnPlay.classList[1] == "play") this.pause();
...
AudioObjConsctructor.prototype.btnClasslistRemoverAdder = function () {
    this.btnPlay.classList.remove("play");
